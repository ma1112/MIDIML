...
def processWav(x,sampleRate, combMatrix = None):
	...
	# used variables
	sampleLength = 525 # in samples ( 44100 is not a multiple of 512...)
	...
	if np.mod(noteDurationinSamples, sampleLength) != 0:
	...
	samplesFromNote = int(noteDurationinSamples / sampleLength) # we are creating this many samples from one note.
	...
	samples = samples.reshape(-1,sampleLength) # reshapes so each row is a sample of a note.
	...
	for i in tqdm(range(samples.shape[0])):
		cqt = librosa.core.cqt(samples[i,:].astype(np.float),sampleRate,512)[:,0]
		cqt = librosa.core.logamplitude(cqt)
		cqts.append(cqt)
	cqts = np.asarray(cqts).reshape(samples.shape[0],-1)
